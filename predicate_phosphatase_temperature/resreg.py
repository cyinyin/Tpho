...
def bin_split(y, bins):
    ...
    Example
    --------
    >>> X = np.random.uniform(0, 1, size=(100,10))  # 从一个均匀分布[low,high)中随机采样
    >>> y = np.random.uniform(0, 1, size=100)  # 返回值：ndarray类型，其形状和参数size中描述一致
...
def uniform_test_split(X, y, bins, bin_test_size=0.5, verbose=False, random_state=None):
    ...
    将阵列或矩阵拆分为训练和测试子集，以使测试集大致一致（即每个仓中的样本数量相同）。
    Parameters
    -----------
    ...
    bins : array_like
        A one-dimensional and monotonically increasing array of boundary values for 
        splitting y into bins.一个一维单调递增的边界值数组，用于将y拆分为二进制。
    bin_test_size : int or float (default=0.5)
        ...
        the smallest bin.如果是int，bin_test_size是从每个bin中抽取的样本数统一测试集。如果float（介于0和1之间），则为最小仓位大小的分数。
    verbose : bool, optional (default=False)
        If True, print a dataframe showing the range and the frequency of each bin for
        the training and testing set.如果为True，则打印一个数据帧，显示每个bin的范围和频率培训和测试集
    random_state : int or None, optional (default=None)
        If int, random_state is the seed used by the random number generator. If None, 
        the random number generator is the RandomState instance used by np.random.
        如果是int，random_state是随机数生成器使用的种子。如果为None，则随机数生成器是np.random使用的RandomState实例。
...
class Rebagg():
    ...
    Rebagg实现了重采样装袋，以处理回归问题中的数据不平衡。将装袋集合中的每个回归量拟合到重新采样的数据集。重采样可以通过随机过采样、SMOTER、高斯噪声的添加或WERCS算法来实现。
    
    Parameters
    ------------
    m : int (default=100)
        Number of models (regressors) in the ensemble(集合中模型的数量)
    ...
    def sample(self, X, y, relevance, relevance_threshold,
        ...
        重新采样数据集并返回一个较小的平衡数据集，用于拟合集合中的单个回归器。
        Parameters
        ------------
        X : array-like or sparse matrix 
        ...
        relevance_threshold : float
            ...
            rare domain.
            形成罕见和正常域的相关性阈值。相关性小于correlance_threshold的目标值形成正常域，
            而相关性大于或等于correlace_threshold的目标值则形成罕见域。
        ...
        [X_reg, y_reg] : list
            ...
            列出重新采样的包含特征（X_reg）和目标值（y_reg）的列表用于拟合集合中单个回归器的数据集。
        """
    ...
    def fit(self, X, y, relevance, relevance_threshold=0.5, sample_method='random_oversample',
        ...
        for i in range(self.m):
            ...
            for te in y_reg:
                if te >= 0 and te <= 30:
                    p1 += 1
                elif te >= 30 and te <= 50:
                    p2 += 1
                elif te >= 50 and te <= 65:
                    p3 += 1
                elif te >= 65 and te <= 85:
                    p4 += 1
                else:
                    p5 += 1
            plt.rcParams['font.sans-serif'] = ['SimHei']
            plt.rcParams['axes.unicode_minus'] = False
            x_list = ['0-30', '30-50', '50-65', '65-85', '85-120']
            number = [p1, p2, p3, p4, p5]
            mm = plt.bar(x_list, number)
            plt.bar_label(mm, label_type='edge')
            ...
            regressors[i].fit(X_reg, y_reg)  # .astype('int')
