...
class _BrendaBaseClass(object):
    ...
    def _normalize_name(self, organism):
        ...
        '''
        assert type(
            organism) is str, 'Error, the organism names must be supplied as strings. The input "%s" is not.' % organism
    ...
    def _get_table_divs(self):
        ...
        if table is None:
            ...
            assert any([self.table_name in s.get_text() for s in
                        table.find_all("div", {"class": "header"})]), 'Something is wrong with %s' % self.table_name
                    ...
                    if data[organism] != {}:
                        for uniprot_id in data[organism].keys():
                            ...
                            for information in data[organism][uniprot_id].keys():
                                ...
                                combined_data[organism][uniprot_id][information].extend(
                                    data[organism][uniprot_id][information])
...
class Organism(_BrendaBaseClass):
    ...
    def _parse_single_div(self, div):
        ...
        if len(text.split('\n')) > 3:
            ...
            if organism.lower().strip().startswith(
                    'no activity'):  # sometimes the organism field says, no activity in ... , get rid of this
                return None
            ...
            # get a list of the uniprot ids, filter out comments and such by matching to the potential ones
            uniprot_id_list = get_identifiers_from_html(parts[4].upper())
            uniprot_id_list = [s for s in uniprot_id_list if
                               (s in potential or get_identifiers_from_html(s) != []) and s != u'']
